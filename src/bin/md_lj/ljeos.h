...
#define ljeos3d_mbwr_jzg(rho, tp, pres, fex, muex) \
  ljeos3d_mbwr(rho, tp, pres, fex, muex, LJEOS_MBWR_JZG, NULL)

#define ljeos3d_mbwr_kn(rho, tp, pres, fex, muex) \
  ljeos3d_mbwr(rho, tp, pres, fex, muex, LJEOS_MBWR_KN, NULL)



/* compute reference thermal dynamics variables
   using the modified Benedict-Webb-Rubin (MBWR) equation of states
   return the average potential energy
   *pres:  pressure
   *fex: Helmholtz free energy (potential part)
   *muex: Gibbs free energy (potential part) */
INLINE real ljeos3d_mbwr(real rho, real tp, real *pres,
    real *fex, real *muex, int eos, const double *x)
{
  /* Reference:
  ...
  /* default parameters */
  if ( eos == LJEOS_MBWR_JZG ) {
    x = ljparam_jzg1993;
  } else if (eos == LJEOS_MBWR_KN ) {
    x = ljparam_kn1994;
  }

  gam = x[33];
  /* Cf. JZG 1993, Table 5 */
  a[0] = x[1]*tp + x[2]*sqrt(tp) + x[3] + x[4]/tp + x[5]/(tp*tp);
  a[1] = x[6]*tp + x[7] + x[8]/tp + x[9]/(tp*tp);
  a[2] = x[10]*tp + x[11] + x[12]/tp;
  a[3] = x[13];
  a[4] = x[14]/tp + x[15]/(tp*tp);
  a[5] = x[16]/tp;
  a[6] = x[17]/tp + x[18]/(tp*tp);
  a[7] = x[19]/(tp*tp);
  /* Cf. JZG 1993, Table 6 */
  b[0] = (x[20] + x[21]/tp)/(tp*tp);
  b[1] = (x[22] + x[23]/(tp*tp))/(tp*tp);
  b[2] = (x[24] + x[25]/tp)/(tp*tp);
  b[3] = (x[26] + x[27]/(tp*tp))/(tp*tp);
  b[4] = (x[28] + x[29]/tp)/(tp*tp);
  b[5] = (x[30] + x[31]/tp + x[32]/(tp*tp))/(tp*tp);
  /* Cf. JZG 1993, Table 8 */
  c[0] = x[2]*sqrt(tp)/2 + x[3] + 2*x[4]/tp + 3*x[5]/(tp*tp);
  c[1] = x[7] + 2*x[8]/tp + 3*x[9]/(tp*tp);
  c[2] = x[11] + 2*x[12]/tp;
  c[3] = x[13];
  c[4] = 2*x[14]/tp + 3*x[15]/(tp*tp);
  c[5] = 2*x[16]/tp;
  c[6] = 2*x[17]/tp + 3*x[18]/(tp*tp);
  c[7] = 3*x[19]/(tp*tp);
  /* Cf. JZG 1993, Table 9 */
  d[0] = (3*x[20] + 4*x[21]/tp)/(tp*tp);
  d[1] = (3*x[22] + 5*x[23]/(tp*tp))/(tp*tp);
  d[2] = (3*x[24] + 4*x[25]/tp)/(tp*tp);
  d[3] = (3*x[26] + 5*x[27]/(tp*tp))/(tp*tp);
  d[4] = (3*x[28] + 4*x[29]/tp)/(tp*tp);
  d[5] = (3*x[30] + 4*x[31]/tp + 5*x[32]/(tp*tp))/(tp*tp);
  ...
  Pt = rho*(tp + Pa + F*Pb); /* Cf. JZG 1993, Eq. (7) */
  if (fex) *fex = (real) Aex;
  if (pres) *pres = (real) Pt;
  if (muex) *muex = (real) (Aex + Pt/rho - tp);
  return (real) u;
}
 ...
 * Integrate[(1-Exp[-(4/x^12-4/x^6+1)/tp]), {x, 0, 2.0^(1.0/6)}]
 * */
INLINE double ljeos3d_dhBH(double tp, double *dfdbeta)
{
  ...
  x = sqrt(tp);
  *dfdbeta = Ci[2] + 0.5*Ci[1]*x - 0.5*C1*x*tp - tp*Cln;
  return Ci[2]/tp + Ci[1]/x + Ci[0] + C1*x + Cln*log(tp);
}
 ...
 * Parameters are given by Table 2 with the functional form given by (29) */
INLINE double ljeos3d_dB2hBH(double tp, double *dfdbeta)
{
  /* from Table 2 */
  const double Ci[8] = {
    0.02459877,
 ...
 * http://w.sklogwiki.org/SklogWiki/index.php/FORTRAN_code_for_the_Kolafa_and_Nezbeda_equation_of_state
 */
INLINE real ljeos3d_pvehbh(real rho, real tp,
    real *pres, real *fex, real *muex)
{
  /* Table 3 */
  ...
  double invx = 1/sqrt(tp);
  double xprho, xpT, xp;
  int i, j;

  dia = ljeos3d_dhBH(tp, &ddia);
  dB2 = ljeos3d_dB2hBH(tp, &ddB2);
  grho2 = gam*rho*rho;
  xpngrho2 = exp(-grho2);
  eta = PI*rho*dia*dia*dia/6;
  Ahs = tp*ljeos3d_bAhs(eta);
  AhBH = xpngrho2*rho*tp*dB2;
  ACij = zCij = uCij = 0;
  for ( i = 0; i <= 4; i++ ) {
    xpT = pow(invx, i);
    xprho = rho;
...
/* default */
#define ljeos3d_get(rho, tp, pres, fex, muex) \
  ljeos3d_getx(rho, tp, pres, fex, muex, LJEOS_PVEHBH_KN)

INLINE real ljeos3d_getx(real rho, real tp, real *pres,
    real *fex, real *muex, int eos)
{
  if ( eos == LJEOS_MBWR_JZG || eos == LJEOS_MBWR_KN ) {
    return ljeos3d_mbwr(rho, tp, pres, fex, muex, eos, NULL);
  } else if ( eos == LJEOS_PVEHBH_KN ) {
    return ljeos3d_pvehbh(rho, tp, pres, fex, muex);
  } else {
